...
public class PlaylistDaoImpl implements PlaylistDao {
  ...
  public ArrayList<PlaylistEntity> postNewPlaylistAndReturnAll(PlaylistRequest newPlaylist, int userId) {
    try (Connection conn = connection()) {
      PreparedStatement statement = conn
          .prepareStatement("INSERT INTO Playlists(PlaylistName, PlaylistOwnerId) VALUES (?, ?)");
  ...
  public ArrayList<TrackEntity> getTracksFromPlaylistId(int playlistId) {
    ...
    try (Connection conn = connection()) {
      ...
      PreparedStatement playlist_tracks_statement = conn.prepareStatement(
          "SELECT * FROM Playlist_tracks INNER JOIN Tracks ON Playlist_Tracks.TrackId = Tracks.TrackId WHERE Playlist_Tracks.PlaylistId = ?");
      ...
      while (playlist_tracks_results.next()) {
        tracks.add(trackDao.getTrackInformationWithinPlaylist(
            Integer.parseInt(playlist_tracks_results.getString("TrackId")), playlistId));
      }
    } catch (SQLException e) {
      e.printStackTrace();
    }
  ...
  public void addTrackToPlaylist(int playlistId, TrackEntity track) {
    try (Connection conn = connection()) {
      PreparedStatement statement = conn
          .prepareStatement("INSERT INTO Playlist_Tracks (PlaylistId, TrackId, TrackOfflineAvailableInPlaylist) " +
              "VALUES(?, ?, ?)");
      statement.setInt(1, playlistId);
  ...
  public void removeTrackFromPlaylist(int playlistId, int trackId) {
    try (Connection conn = connection()) {
      PreparedStatement statement = conn
          .prepareStatement("DELETE FROM Playlist_Tracks WHERE PlaylistId = ? AND TrackId = ?");
      statement.setInt(1, playlistId);
